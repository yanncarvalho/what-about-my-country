...
class BasicView(View):
    ...
    def http_method_not_allowed(self, request, *args, **kwargs) -> ErrorResponse:
        """error handler to not allowed method"""
        return _error_request(f'{request.method} method not allow to this path.', 405)

    @staticmethod
    def _sanitize_response(response: Dict[str, Union[str, int, float]]) -> Tuple[GraphQLResp, int]:
...
class CountryViewVersionOne(BasicView):
    ...
    def post(self, request) -> GraphQLResp:
        ...
        result, status_code = BasicView._sanitize_response(response)
